namespace LazySloth.Validation
{
    ...
    {
        public static List<string> GetAssetsPathsByFilter(string filter, string path)
        {
        ...
        public static bool FieldIsVisibleInInspector(FieldInfo fieldInfo)
        {
            ...
            if (isNotSerializedField || isDictionary || isAction)
            {
                return false;
            }
            ...
            var isSerializeField = HasCustomAttribute<SerializeField>(fieldInfo) && !HasCustomAttribute<HideInInspector>(fieldInfo);
